...
def transform_camera_view(XYZ, sensor_height, camera_elevation_degree):
    ...
    """
    R = ru.get_r_matrix(
        np.asarray([1.0, 0.0, 0.0]), rotation_angle=np.deg2rad(camera_elevation_degree)
    )
...
def transform_pose(XYZ, current_pose):
    ...
    """
    R = ru.get_r_matrix(
        np.asarray([0.0, 0.0, 1.0]), rotation_angle=current_pose[2] - np.pi / 2.0
    )
    XYZ = np.matmul(XYZ.reshape(-1, 3), R.T).reshape(XYZ.shape)
...
def get_point_cloud_from_z_t(
    ...
    grid_x = grid_x.expand(Y_t.size())
    grid_z = grid_z.expand(Y_t.size())
...
def transform_pose_t(XYZ: Tensor, current_pose: np.ndarray, device: str) -> Tensor:
    """
    Transforms the point cloud into geocentric frame to account for
    camera position
    Input:
