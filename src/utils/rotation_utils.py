...
def get_r_matrix(ax: ndarray, rotation_angle: float) -> ndarray:
    ax_normalized = normalize(ax)
    if np.abs(rotation_angle) > ANGLE_EPS:
        S_hat = np.array(
            [
                [0.0, -ax_normalized[2], ax_normalized[1]],
                [ax_normalized[2], 0.0, -ax_normalized[0]],
                [-ax_normalized[1], ax_normalized[0], 0.0],
            ],
            dtype=np.float32,
        )
        R = (
            np.eye(3)
            + np.sin(rotation_angle) * S_hat
            + (1 - np.cos(rotation_angle)) * (np.linalg.matrix_power(S_hat, 2))
        )
    else:
        R = np.eye(3)
    return R
...
def r_between(v_from_: ndarray, v_to_: ndarray) -> ndarray:
...
def rotate_camera_to_point_at(
    up_from: ndarray, lookat_from: ndarray, up_to: ndarray, lookat_to: ndarray
) -> ndarray:
